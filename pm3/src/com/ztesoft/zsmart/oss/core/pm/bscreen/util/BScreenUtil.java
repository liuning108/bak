...
public class BScreenUtil {
     ...
     * logger
     */
    private static ZSmartLogger  logger = ZSmartLogger.getLogger(BScreenUtil.class);
    ...
    */
    ...
    public static Map dic2Map2(DynamicDict dynamicDict) {
        ...
        for (String key : dynamicDict.valueMap.keySet()) {
            Object value = dynamicDict.valueMap.get(key);
            if (value instanceof DynamicDict) {
                map.put(key, BScreenUtil.dic2Map2((DynamicDict) value));
            }
            else {
                if (value instanceof ArrayList) {
                    ...
                    for (DynamicDict dict : list) {
                        Map m = BScreenUtil.dic2Map2(dict);
                        maps.add(m);
                    }
                    map.put(key, maps);
                }
     ...
     * @throws Exception 
     */ 
    public static String svgMapToJson(String path) throws Exception {
        List<JSONObject> result =new ArrayList<JSONObject>();
        System.out.println(path);
        String fileDirectory = ConfigurationMgr.instance().getString("upload.uploadFileDirectory");
        String fileFullPath = fileDirectory + "/" + path;
        File file = new File(fileFullPath);
         ...
         Element element = document.getRootElement().element("g");
         for (Element pathElement : element.elements("path")) {
             String id =pathElement.attribute("id").getText();
             String d =pathElement.attribute("d").getText();
             String title =id;
             if(pathElement.attribute("title")!=null) {
                 title =pathElement.attribute("title").getText();
             }
