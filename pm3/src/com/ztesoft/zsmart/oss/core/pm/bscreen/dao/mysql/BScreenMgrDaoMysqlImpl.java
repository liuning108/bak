package com.ztesoft.zsmart.oss.core.pm.bscreen.dao.mysql;
...
import com.ztesoft.zsmart.oss.core.pm.bscreen.restful.client.DashBoardAPIClient;
 ...
 */
public class BScreenMgrDaoMysqlImpl extends BScreenMgrDao {
    ...
    private String saveImage(String base64, String topicId) {
       ...
       System.out.println("saveImage:"+path);
    ...
    private void updateTopicNodes(DynamicDict data) throws BaseAppException {
        ...
        // TODO 删除所有子节点
        String delsql = "delete from pm_bscreen_topic_nodes  " + " where topic_no=?";
    ...
    public boolean deleteBScreenById(String id) throws BaseAppException {
        ...
        try {
            String sql = "DELETE FROM PM_BSCREEN_TOPIC_LIST  WHERE TOPIC_NO=?";
            ParamArray pa = new ParamArray();
            pa.set("", id);
            this.executeUpdate(sql, pa);
            String deleteNodeSql = "DELETE FROM PM_BSCREEN_TOPIC_NODES  WHERE TOPIC_NO=?";
            ParamArray pa2 = new ParamArray();
            pa2.set("", id);
    ...
    private HashMap<String, String> getApiSource(String sourceID) throws BaseAppException {
        // TODO Auto-generated method stub
        String sql = "select URL from  opbs_itf_system where SYSTEM_ID=?";
        ParamArray pa = new ParamArray();
        pa.set("", sourceID);
    ...
    private void deleteSourceServiceAttrs(Map<String, String> map) throws BaseAppException {
        // TODO Auto-generated method stub
        String sql = "delete FROM  PM_BSCREEN_SERVICE_COL where service_no=?";
        ParamArray pa = new ParamArray();
        String no = map.get("no");
        pa.set("", no);
        this.executeUpdate(sql, pa);
    ...
    public Map<String, Object> delSourceServiceById(Map<String, String> param) throws BaseAppException {
        ...
        ParamArray pa = new ParamArray();
        pa.set("", param.get("Id"));
        String subSql = "DELETE  FROM PM_BSCREEN_SERVICE_COL  WHERE SERVICE_NO=?";
        String parentSql = "DELETE FROM PM_BSCREEN_SERVICE_LIST  WHERE SERVICE_NO=?";
    ...
    public Map<String, Object> getServerSkeleton(Map<String, String> param) throws BaseAppException {
        ...
        Map<String, Object> sqlDatas ;
        List<Map<String, Object>> datas;
        if("1".equalsIgnoreCase((""+json.get("type")).trim())){
            info = getSource(source);
            sqlDatas = SQLUtil.getDatas(SQLUtil.getJdbcTemplate(info), sql);
            datas = (List<Map<String, Object>>) sqlDatas.get("datas");
        }else{
            info = getApiSource(source);
            String url = info.get("URL")+sql;
            System.out.println("URL:"+url);
            Map<String,Object> callResult =DashBoardAPIClient.call(url, DashBoardAPIClient.DATATYPE);
            datas = (List<Map<String, Object>>)callResult.get("data");
    ...
    public List<HashMap<String, String>> getApiSource() throws BaseAppException {
        String sql = "select SYSTEM_ID ID ,SYSTEM_NAME NAME, URL  from  opbs_itf_system";
        ParamArray params = new ParamArray();
        return queryList(sql, params);
    }
    ...
    public Map<String, Object> getAPIField(Map<String, String> param) throws BaseAppException {
        String source = param.get("source");
        String sql = param.get("sql");
        Map<String, String> info = getApiSource(source);
        if (info == null) {
            Map<String, Object> error = new HashMap<String, Object>();
            error.put("message", "Data Source Error");
            error.put("fields", new ArrayList<String>());
        ...
        String url = info.get("URL")+sql;
        return DashBoardAPIClient.call(url, DashBoardAPIClient.METATYPE);
    ...
    public List<Map<String, Object>> queryBScreenList() throws BaseAppException {
        String sql = "SELECT  T.TOPIC_NO as ID ,T.TOPIC_NAME as NAME,ATTRS,T.IMAGE_PATH as IMAGE_PATH,"
            + "T.IS_SHARE ,T.OPER_USER,T.OPER_DATE  FROM PM_BSCREEN_TOPIC_LIST T  ORDER BY T.OPER_DATE";
        ParamArray pa = new ParamArray();
        List<Map<String, Object>> result = BScreenUtil.toConvert(this.queryList(sql,pa));
        for (Map<String, Object> map : result) {
            String att = "" + map.get("attrs");
    ...
    public String addMap(String filename, String filePath, String json) throws BaseAppException {
        String id = BScreenUtil.getSeq("PM_BSTOPIC_SEQ");
        String sql = ""
            + "INSERT INTO pm_areamap_spec ( "
            + "    areamap_no, "
            + "    areamap_name, "
            + "    areamap_url, "
            + "    is_display, "
        ...
        ParamArray pa  =new ParamArray();
        pa.set("", id);
        pa.set("", BScreenUtil.getFileNameNoEx(filename));
        pa.set("", filePath);
        this.executeUpdate(sql, pa);
        String sql2 = ""
            + "INSERT INTO pm_areamap_attr_spec ( "
            + "    areamap_no, "
            + "    attr_seq, "
            + "    attrs "
            + ") VALUES ( "
            ...
            + "    ? "
            + ")";
        List<String> jsons= BScreenUtil.splitByNumbers(json, 2000);
       for(int i=0;i<jsons.size();i++) {
           String json_part = jsons.get(i);
           ParamArray pa2 = new ParamArray();
           pa2.set("", id);
           pa2.set("", i);
           pa2.set("", json_part);
           this.executeUpdate(sql2, pa2);
       }
    ...
    public String getMap(String id) throws BaseAppException {
        // TODO Auto-generated method stub <br>
        String sql = ""
            + "select attr_seq,attrs from   PM_AREAMAP_ATTR_SPEC  t  where t.AREAMAP_NO= ? order by attr_seq";
        ParamArray pa = new ParamArray();
        pa.set("", id);
        StringBuffer attrs= new StringBuffer();
        for ( HashMap<String,String> s:  this.queryList(sql, pa)) {
    ...
    public void delMap(String id) throws BaseAppException {
        String sql1 = ""
            + "delete  from PM_AREAMAP_ATTR_SPEC where AREAMAP_NO= ? ";
         String sql2 = ""
            + "delete  from  PM_AREAMAP_SPEC where AREAMAP_NO= ? ";
         ParamArray pa = new ParamArray();
         pa.set("", id);
    ...
    public List<HashMap<String, String>> getMapList() throws BaseAppException {
        // TODO Auto-generated method stub <br>
        String sql = ""
            + "select AREAMAP_NO NO, AREAMAP_NAME NAME,AREAMAP_URL PATH "
            + "from PM_AREAMAP_SPEC "
            + "where IS_DISPLAY='1' and SCOPE_MODEL='2'  "
            + "order by DISPLAY ,AREAMAP_NAME";
    ...
    public void renameMap(String id, String name) throws BaseAppException {
        // TODO Auto-generated method stub <br>
        String sql ="update pm_areamap_spec set AREAMAP_NAME=?  WHERE AREAMAP_NO= ? ";
        ParamArray pa = new ParamArray();
        pa.set("", name);
        pa.set("", id);
        this.executeUpdate(sql, pa);
