 ...
 */
define([
        'text!oss_core/inms/pm/adhoc/templates/ChartContainer.html',
        "oss_core/inms/pm/adhoc/assets/js/echarts",
        ...
        "oss_core/inms/pm/adhoc/views/DrillEntranceWin",
        "oss_core/inms/pm/adhoc/views/ViewExtensionDialog",
        'i18n!oss_core/inms/pm/adhocdesigner/i18n/adhoc',
        "oss_core/inms/pm/adhoc/assets/js/pivot",
        "oss_core/inms/pm/adhoc/assets/js/echarts-wordcloud.min",
        'css!oss_core/inms/pm/adhoc/assets/js/pivot.css'
    ],
    function(mainTpl, echarts, echarts3, adhocUtil, action, pmUtil, drillEntranceWin, viewExtensionDialog, i18nData) {
    return fish.View.extend({
        ...
        events: {
            ...
            'click .filter-minus-btn': 'filterMinusClick',
            'click .sort-minus-btn': 'sortMinusClick'
        },
        ...
        initialize: function (opt) {
            ...
            this.axisCfgXaxis_bak = ""; // x轴备份 用于维度扩展后恢复默认维度
            this.groupDimNoList_bak = []; // 饼系图表维度备份 用于维度扩展后恢复默认维度
            ...
            this.groupExtendDimList = []; // 扩展分析的字段集合
            this.extendDimFilterList = []; // 扩展分析的当前维度过滤条件集合
            this.groupExtendDimFilterList = []; // 扩展分析的当前维度过滤条件集合
            //
        ...
        afterRender: function () {
            ...
            this.$('#ad-showdisplayfield-btn-'+this.chart_id).on("click", function(){
                that.showdisplayfieldCfg();
            });
            this.$('#ad-datafilter-btn-'+this.chart_id).off();
            this.$('#ad-datafilter-btn-'+this.chart_id).on("click", function(){
                that.showDataFilterCfg();
            });
            this.$('#ad-sort-btn-'+this.chart_id).off();
            this.$('#ad-sort-btn-'+this.chart_id).on("click", function(){
                that.showSortCfg();
            });
            this.$('#ad-pivottable-btn-'+this.chart_id).off();
            this.$('#ad-pivottable-btn-'+this.chart_id).on("click", function(){
                that.showPivotTable();
            });
            this.$('#ad-datafilter-filterconditype-'+this.chart_id).off();
            this.$('#ad-datafilter-filterconditype-'+this.chart_id).on("change", function(){
                that.filterCondiType = that.$('#ad-datafilter-filterconditype-'+that.chart_id).val();
            });
            this.$('#ad-dimfilter-fieldsel-'+this.chart_id).off();
            this.$('#ad-dimfilter-fieldsel-'+this.chart_id).on("change", function(){
                var fieldId = that.$('#ad-dimfilter-fieldsel-'+that.chart_id).val();
                for(var i=0;i<that.selectedDimIndiList.length; i++){
                    var dimIndiObj = that.selectedDimIndiList[i];
                    if(dimIndiObj.COL_INDEX == fieldId){
                        that.colType = dimIndiObj.COL_TYPE;
                        if(that.colType == "00" || that.colType == "02"){
                            that.META_DIM_CODE = dimIndiObj.META_DIM_CODE;
                            that.showDimFilterSelect();
                            that.filterFieldType = "0";
                        }else if(that.colType == "01"){
                            that.showKpiFilterSelect();
                            that.filterFieldType = "1";
                        }
                    }
                }
            });
        ...
        refreshCss: function (switchForExtendedAnalysis) {
            if(this.chart_type=="grid"){
                ...
                if(this.isCompareAnalysis || switchForExtendedAnalysis){
        ...
        showCondiSelectWin: function () {
            ...
            require([
                ...
                self.listenTo(dialog, 'okDimSelectEvent', function (data) {
                    ...
                    self.$('#ad-dimfilter-condivalue').val(selectedNameStr);
                });
        ...
        showPivotDialog: function () {
            ...
            require([
                'oss_core/inms/pm/adhoc/views/PivotDialog'
            ], function (Dialog) {
                var dialog = new Dialog({
                ...
                self.pivotDialog = fish.popup(option);
                dialog.contentReady();
                self.listenTo(dialog, 'cancelEvent', function () {
                    self.pivotDialog.close();
                });
            });
        },
        ...
        addFilterHtml: function(fmtId, filterCondiName) {
            ...
            var htmlText = '<div name="' + divId + '" class="form-group"><label class="col-md-2 col-md-offset-2" style="text-align: left;padding-left:25px">' + fieldName
                ...
                + filterCondiName + '</div><div class="col-md-1" style="padding-left:15px"><button name="' + divId + '" type="button" class="btn btn-ico filter-minus-btn"><i class="fa fa-minus"></i></button></div></div>';
            this.$('#ad-filter-container').append(htmlText);
            return divId;
        },

        filterMinusClick: function(e) {
            var self = this;
            var itemId = e.currentTarget.name;
        ...
        addSortRule: function() {
            ...
            var htmlText = '<div name="'+divId+'" class="form-group"><label class="col-md-3 col-md-offset-2 control-label" style="text-align: left;padding-left:25px">' + sortFieldName
                ...
                + '<div class="col-md-1"><button name="'+divId+'" type="button" class="btn btn-ico sort-minus-btn"><i class="fa fa-minus"></i></button></div></div>';
            this.$('#ad-sort-container').append(htmlText);
            self.sortList[self.sortList.length] = {
                itemId: divId,
                sortField: sortField,
                sortType: sortType
            }
        ...
        sortMinusClick: function(e) {
            var self = this;
            var itemId = e.currentTarget.name;
            self.$("div[name="+ itemId +"]").remove();
            for(var i=0; i<self.sortList.length; i++){
                var sortObj = self.sortList[i];
                if(sortObj.itemId == itemId){
                    self.sortList.splice(i,1);
                    break;
                }
            }
        },
        ...
        getDoubleAxisTypeLegendData: function () {
            var self = this;
            var legendData = [];
            fish.forEach(self.firstAxisKpiList, function (kpi) {
                fish.forEach(self.colModel, function (col) {
        ...
        getLineTypeXaxisData: function () {
            ...
            fish.forEach(this.simuDp, function (data) {
                var dataItem = data[self.axisCfgXaxis];
                var isExist = false;
        ...
        getPieChartOption:  function (param, groupIndex) {
                            ...
                            normal: {
                                label : {
                                    ...
                                    formatter:function(params){
                                        var yAxisIndex = self.KPI_NO;
                                        var isThousandDisplay = false;
                                        fish.forEach(self.selectedDimIndiList, function(kpiCfgObj){
                                            if(kpiCfgObj.COL_INDEX==yAxisIndex && (kpiCfgObj.isThousandDisplay=="true" || kpiCfgObj.isThousandDisplay)){
                                                isThousandDisplay = true;
                                            }
                                        });
                                        if(isThousandDisplay){
                                            return self.sliceDecimal(params.data.value);
                                        }else{
            ...
            if(this.pieChartType=="1"){
                this.option.series[0].radius = ['25%', '60%'];
            }
            return this.option;
        },
                        ...
                        normal: {
                            label: {
                                ...
                                formatter:function(params){
                                    ...
                                    if(isThousandDisplay){
                                        return self.sliceDecimal(params.value);
                                    }else{
                                        return params.value;
                                    }
                                },
                    ...
                    itemStyle: {
                        normal: {
                            label : {
                                color: '#555',
                                formatter:function(params){
                                    var yAxisIndex = self.axisCfgYaxisList[params.seriesIndex];
                                    var isThousandDisplay = false;
                                    fish.forEach(self.selectedDimIndiList, function(kpiCfgObj){
                                        if(kpiCfgObj.COL_INDEX==yAxisIndex && (kpiCfgObj.isThousandDisplay=="true" || kpiCfgObj.isThousandDisplay)){
                                            isThousandDisplay = true;
                    ...
                    itemStyle: {
                        normal: {
                            label : {
                                color: '#555',
                                formatter:function(params){
                                    var xaxis_field_isthousanddisplay = false;
                                    var yaxis_field_isthousanddisplay = false;
                                    fish.forEach(self.selectedDimIndiList, function(kpiCfgObj){
                                        if(kpiCfgObj.COL_INDEX==self.xaxis_field && (kpiCfgObj.isThousandDisplay=="true" || kpiCfgObj.isThousandDisplay)){
                                            xaxis_field_isthousanddisplay = true;
                                        }
                                        if(kpiCfgObj.COL_INDEX==self.yaxis_field && (kpiCfgObj.isThousandDisplay=="true" || kpiCfgObj.isThousandDisplay)){
                                            yaxis_field_isthousanddisplay = true;
                                        }
                                    });
                                    var valueArray = params.value;
                                    var newValueArray = [];
                                    if(xaxis_field_isthousanddisplay){
                                        newValueArray[newValueArray.length] = self.sliceDecimal(valueArray[0]);
                                    }else{
                                        newValueArray[newValueArray.length] = valueArray[0];
                                    }
                                    if(yaxis_field_isthousanddisplay){
                                        newValueArray[newValueArray.length] = self.sliceDecimal(valueArray[1]);
                                    }else{
                                        newValueArray[newValueArray.length] = valueArray[1];
                                    }
            ...
            if(isPager){
                opt = {
                    ...
                    gridComplete: function (e) {//数据加载完成触发的事件
                        self.$('[name="ad-grid-drill-cell"]').off();
                        self.$('[name="ad-grid-drill-cell"]').on("click", function(e){
                            self.cellLinkClick(e);
                        });
                        self.$('#ad-grid-container-'+self.chart_id).jqGrid("mergeColCellByData", self.mergeCols); //指定 第4，6，7列相同数据进行合并
                    }
                };
                this.$('#ad-grid-pager-'+this.chart_id).show();
                this.$('#ad-grid-pager-'+this.chart_id).pagination({
                    records: 0,
                    onPageClick: function (e, eventData) {
                        self.gridPageClick(eventData.page);
                    }
                });
            }else{
                opt = {
        ...
        // 表格调用后台翻页
        gridPageClick: function (page) {
            this.rowNumOfPager = this.$('#ad-grid-pager-'+this.chart_id).pagination("option","rowNum");
            this.startOfPager = page;
        ...
        cellLinkClick: function(e){
            ...
            require([
                'oss_core/inms/pm/counter/views/CounterQry'
            ], function (counterQry) {
                var view = new counterQry(drillObj);
                self.$('#ad-grid-drill-content-'+self.chart_id).html(view.render().$el);
                view.doComplete();
                self.listenTo(view, 'exitCountQry', function (data) {
                    self.exitCellDrill();
                });
                ...
                fish.forEach(self.selectedDimIndiList, function(kpiCfgObj){
                    if(kpiCfgObj.COL_TYPE=="01"){
                        if(kpiCfgObj.hasOwnProperty("precision")){
                            data[kpiCfgObj.COL_INDEX] = self.definedRound(Number(data[kpiCfgObj.COL_INDEX]),kpiCfgObj.precision);
                        }
                        if((kpiCfgObj.isThousandDisplay=="true" || kpiCfgObj.isThousandDisplay) && self.chart_type=="grid"){
        ...
        updateGroupSimuDp: function (dataList, groupIndex) {
            ...
            fish.forEach(dataList, function(data){
                fish.forEach(self.selectedDimIndiList, function(kpiCfgObj){
                    if(kpiCfgObj.COL_TYPE=="01"){
                        if(kpiCfgObj.hasOwnProperty("precision")){
                            data[kpiCfgObj.COL_INDEX] = self.definedRound(Number(data[kpiCfgObj.COL_INDEX]),kpiCfgObj.precision);
        ...
        updateColumnCfg: function (param) {
            ...
            this.chart = echarts.init(this.$("#ad-chart-container-"+this.chart_id)[0]);
            this.chart.setOption(this.getColChartOption(param), true);
            this.bindExtendAnalysis(this.chart);
        },

        bindExtendAnalysis: function (chart) {
            var self = this;
            chart.off();
            if(this.isExtendedAnalysis){
                chart.on('click', function (params) {
                    if(self.viewExtensionWin){
                        self.viewExtensionWin.close();
                    }
                    if(params.event.event.currentTarget.parentElement.name){
                        //group_0
                        params.groupIndex = params.event.event.currentTarget.parentElement.name.substring(6);
                    }
                    self.showDrillEntranceDialog(params);
        ...
        showViewExtensionDialog: function (params) {
            ...
            if(groupIndex){
                ...
                if (!currentDim) {
                    currentDim = this.groupExtendDimList[groupIndex][this.groupExtendDimList[groupIndex].length - 1];
                }
            }else {// 非饼系图表 没有分组
                fish.forEach(this.selectedDimIndiList, function (dimObj) {
            ...
            if(groupIndex){
                var sData = {
                    ...
                    dataList: this.groupSimuDp[groupIndex]
                };
            }else {
                var sData = {
                    leftPosition: leftPosition,
            ...
            this.listenTo(dialog, 'extendDimSelect', function (data) {
                //
                if(data.groupIndex){
                    self.trigger("groupExtendDimSelect",{
        ...
        switchGroupDimExtend: function (e) {
            var self = this;
            //ad-extendstep-DIM_NO-groupIndex"
            var tmpStr =  e.currentTarget.name.substring(14);
            var dimNo = tmpStr.substring(0, tmpStr.indexOf("-"));
            var groupIndex = tmpStr.substring(tmpStr.indexOf("-")+1);
            var extendDimList_new = [];
            var extendDimFilterList_new = [];
            for(var i=0;i<this.groupExtendDimList[groupIndex].length;i++){
                var extendDim = this.groupExtendDimList[groupIndex][i];
                if(extendDim.DIM_CODE!=dimNo){
                    extendDimList_new[extendDimList_new.length] = extendDim;
                    extendDimFilterList_new[extendDimFilterList_new.length] = this.groupExtendDimFilterList[groupIndex][i];
                }else{
                    extendDimList_new[extendDimList_new.length] = extendDim;
                    extendDimFilterList_new[extendDimFilterList_new.length] = this.groupExtendDimFilterList[groupIndex][i];
                    break;
            ...
            this.groupList[groupIndex].DIM_NO = dimNo;
            this.groupExtendDimList[groupIndex] = extendDimList_new;
            this.groupExtendDimFilterList[groupIndex] = extendDimFilterList_new;
            //
            this.$("#ad-chart-"+this.chart_id+"-groupextendstep-"+groupIndex).empty();
            var appendHtml = "<a name='ad-extendstep-all-"+groupIndex+"' class='btn btn-link' type='button' style='color:blue'>" + self.resource.ALL + "</a>";
            this.$("#ad-chart-"+this.chart_id+"-groupextendstep-"+groupIndex).append(appendHtml);
            this.$('[name=ad-extendstep-all-'+groupIndex+']').off();
            this.$('[name=ad-extendstep-all-'+groupIndex+']').on("click", function(e){
                self.restoreDimExtend(e);
            });
            fish.forEach(this.groupExtendDimList[groupIndex], function(dimObj){
                var appendHtml = "<a name='ad-extendstep-"+self.groupList[groupIndex].DIM_NO+"-"+groupIndex+"' class='btn btn-link' type='button' style='color:blue'>" + dimObj.DIM_NAME + "</a>";
                self.$("#ad-chart-"+self.chart_id+"-groupextendstep-"+groupIndex).append(" > " + appendHtml);
                self.$('[name=ad-extendstep-'+self.groupList[groupIndex].DIM_NO+'-'+groupIndex+']').off();
                self.$('[name=ad-extendstep-'+self.groupList[groupIndex].DIM_NO+'-'+groupIndex+']').on("click", function(e){
                    self.switchDimExtend(e);
                });
            });
            self.trigger("queryDataForChartGroup", {
                chart: self,
                groupIndex: groupIndex
            });
        },

        restoreDimExtend: function (e){
            var self = this;
            if(this.groupList.length>0){
                this.restoreGroupDimExtend(e);
            }else{
                this.$("#ad-chart-extendstep-"+this.chart_id).empty();
                this.extendDimList = []; // 扩展分析的字段集合
                this.extendDimFilterList = []; // 扩展分析的当前维度过滤条件集合
                fish.forEach(this.selectedDimIndiList, function(dimObj){
                    if(dimObj.COL_NO == self.axisCfgXaxis_bak){
                        ...
                        if (self.groupList.length > 0) {
                            var groupIndex = e.currentTarget.name.substring(18);
                            self.groupList[groupIndex].DIM_NO = self.axisCfgXaxis;
                            self.trigger("queryDataForChartGroup", {
                                chart: self,
                                groupIndex: groupIndex
                            });
                        }else{
                            self.trigger("queryDataForChart", self);
                        }
                    }
                });
        ...
        restoreGroupDimExtend: function (e){
            var self = this;
            var groupIndex = e.currentTarget.name.substring(18);
            this.$("#ad-chart-"+this.chart_id+"-groupextendstep-"+groupIndex).empty();
            this.groupExtendDimList[groupIndex] = []; // 扩展分析的字段集合
            this.groupExtendDimFilterList[groupIndex] = []; // 扩展分析的当前维度过滤条件集合
            fish.forEach(this.selectedDimIndiList, function(dimObj) {
                if (dimObj.COL_NO == self.groupDimNoList_bak[groupIndex]) {
                    self.groupList[groupIndex].DIM_NO = dimObj.COL_INDEX;
                }
            });
            self.trigger("queryDataForChartGroup", {
                chart: self,
            ...
            self.groupDimNoList_bak[groupIndex] = "";
        },
        ...
        updatePieCfg: function (param) {
            ...
            for (var i = 0; i < this.groupList.length; i++) {
                ...
                this.$("#ad-chart-container-"+this.chart_id).append('<div style="height: ' + this.$('#ad-chart-container-'+this.chart_id).height() + 'px" class="col-md-' + mdValue + '">'+
                +'<div id="ad-chart-'+this.chart_id+'-groupextendstep-'+i+' class="text-left"></div>'
                +'<div id="ad-chart-'+this.chart_id+'-groupcontainer-'+i+' ></div>'
                +'</div>');
                var pieChart = echarts.init(this.$("#ad-chart-"+this.chart_id+"-groupcontainer-"+i));
                param.GROUP_TITLE = group.GROUP_TITLE;
                param.DIM_NO = group.DIM_NO;
                param.KPI_NO = group.KPI_NO;
                pieChart.setOption(this.getPieChartOption(param), true);
            }
        },
        ...
        updateKpiCardCfg: function (param) {
            ...
            for (var i = 0; i < this.groupList.length; i++) {
                ...
                param.DIM_NO = group.DIM_NO;
                param.KPI_NO = group.KPI_NO;
                chart.setOption(this.getKpiCardOption(param), true);
            }
        },
        ...
        updateRadarCfg: function (param) {
            ...
            for (var i = 0; i < this.groupList.length; i++) {
                ...
                param.DIM_NO = group.DIM_NO;
                param.KPI_NO = group.KPI_NO;
                radarChart.setOption(this.getRadarChartOption(param), true);
            }
        },
        ...
        updateLineCfg: function (param) {
            ...
            this.chart = echarts.init(this.$("#ad-chart-container-"+this.chart_id)[0]);
            this.chart.setOption(this.getLineChartOption(param), true);
        ...
        getScatterMapOption: function () {
            ...
            $.getJSON("oss_core/inms/pm/adhocdesigner/assets/js/mapData/geoJson/"+mapUrl, function(data) {
                ...
                console.log('registerMap:'+self.AREAMAP_URL);
                self.mapDataList = [];
                var featureCount = 0;
                ...
                self.chart = echarts3.init(self.$("#ad-chart-container-"+self.chart_id)[0]);
        ...
        updateAreaCfg: function (param) {
            ...
            this.chart = echarts.init(this.$("#ad-chart-container-"+this.chart_id)[0]);
            this.chart.setOption(this.getAreaChartOption(param), true);
            this.bindExtendAnalysis(this.chart);
        },
        ...
        updateBarCfg: function (param) {
            ...
            this.chart_type = "bar";
            this.chart = echarts.init(this.$("#ad-chart-container-"+this.chart_id)[0]);
            this.chart.setOption(this.getBarChartOption(param), true);
            this.bindExtendAnalysis(this.chart);
        },
        ...
        updateTreeCfg: function (param) {
            ...
            this.chart = echarts.init(this.$("#ad-chart-container-"+this.chart_id)[0]);
            this.chart.setOption(this.getTreeChartOption(param), true);
        },
        ...
        updateDuijiBarCfg: function (param) {
            ...
            this.chart = echarts.init(this.$("#ad-chart-container-"+this.chart_id)[0]);
            this.chart.setOption(this.getDuijiBarChartOption(param), true);
            this.bindExtendAnalysis(this.chart);
        },
        ...
        updateScatterCfg: function (param) {
            ...
            this.chart = echarts.init(this.$("#ad-chart-container-"+this.chart_id)[0]);
            this.chart.setOption(this.getScatterChartOption(param), true);
            this.bindExtendAnalysis(this.chart);
        },
        ...
        updateDoubleAxisCfg: function (param) {
            ...
            if(this.firstAxisKpiList.length==0 && this.secondAxisKpiList.length==0){
                fish.forEach(this.colModel, function (colObj) {
                    if(colObj.colType == 1){
                        self.firstAxisKpiList[self.firstAxisKpiList.length] = colObj.name;
                    }
                });
            ...
            this.chart.setOption(this.getDoubleAxisChartOption(param), true);
            this.bindExtendAnalysis(this.chart);
        },
        ...
        updateDuijiColumnCfg: function (param) {
            ...
            this.chart = echarts.init(this.$("#ad-chart-container-"+this.chart_id)[0]);
            this.chart.setOption(this.getDuijiColumnChartOption(param), true);
            this.bindExtendAnalysis(this.chart);
        },
        ...
        updateDuijiAreaCfg: function (param) {
            ...
            this.chart = echarts.init(this.$("#ad-chart-container-"+this.chart_id)[0]);
            this.chart.setOption(this.getDuijiAreaChartOption(param), true);
            this.bindExtendAnalysis(this.chart);
        },
        ...
        updateChartHeight: function (switchForExtendedAnalysis) {
            ...
            if((this.isCompareAnalysis && this.isCompareAnalysis!="false") || switchForExtendedAnalysis){
        ...
        updateGridTop: function () {
            if(this.gridTop==''){
                ...
                this.$('#ad-grid-pager-'+this.chart_id).pagination('update',{'records':this.recordCount,'start':this.startOfPager});
            }else{
                var topSimuDp = [];
        ...
        displayFieldConfirm: function () {
            ...
            fish.forEach(displayFieldCheckboxs, function(chkBox){
                if(chkBox.checked == false){// 取消显示 取消时需要判断是否存在过滤项和排序项为此字段
                    var colIndex = chkBox.value;
                    for(var i=0; i<self.showColList.length; i++){
                        if(self.showColList[i] == chkBox.value){
                            self.showColList.splice(i,1);
                            self.hideColList[self.hideColList.length] = chkBox.value;
                            break;
                        }
                    }
                }else{
                    for(var i=0; i<self.hideColList.length; i++){
                        if(self.hideColList[i] == chkBox.value){
                            self.hideColList.splice(i,1);
                            self.showColList[self.showColList.length] = chkBox.value;
                            break;
                        }
                    }
                }
