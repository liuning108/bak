...
define(["oss_core/inms/pm/graphs/gviews/raphael-min"], function(Snap) {
  ...
  GMemory.prototype.drawChart = function(dom,option,w,h) {
    ...
    function darwScollbarLine(box) {
      ...
      curve.attr({'fill': '#e1e3e4', 'stroke-width': 1, 'stroke': '#d5d8da'})
    ...
    scollbar.attr({'stroke': '#dddddd'})
    var bar_w = (7 / count) * (item_w * 7)
    var bar = paper.rect(item_w/2, item_h + 20 + 5 + 20, bar_w, 21)
    bar.attr({'fill': '#d0d3d9', 'stroke-width': 1, 'stroke': '#787a7c', 'opacity': 0.5})
    ...
    function item_rect(x, y, item_w, item_h, text, flag, value) {
      ...
      bk.attr({'fill': '#dcdcdc', 'stroke-width': 0, 'stroke': '#fff'})
      ...
      var handelClick = function() {
        if (selItem.item) {
          try {
            ...
            selItem.item.attr({'height': item_h, 'width': item_w, 'y': y})
            selItem.per.attr({'height': item_h, 'y': y})
            selItem.t.attr({
              'y': item_h + 30
            })
            //var selItemLine =selItem.item.line.getBBox();
            selItem.item.line.show()
            selItem.item.up.item.attr({width: item_w})
            selItem.item.up.line.attr({'stroke-dasharray': '. '})
        ...
        if (item.up && item.up.line) {
          ...
          item.up.line.attr({'stroke-dasharray': null});
        }
        t.attr({
          'y': 6
        })
      ...
      item.attr({'fill': '#00a4f8', 'stroke-width': 0, 'stroke': '#fff'}).hover(hoverIn, hoverOut).click(handelClick)
      item.bk = bk;
      item.line = paper.rect(x + item_w - 4, y, 0.1, item_h);
      item.line.attr({'fill': 'red', 'stroke': '#fff', 'stroke-width': 1, 'stroke-dasharray': '. '})
      ...
      per.attr({'fill': '#e32133', 'stroke-width': 0}).hover(hoverIn, hoverOut).click(handelClick)
      per.attr({"cursor": "pointer"});
      per.animate({
        'width': perW
      }, 500)
      var t = paper.text(x + item_w / 2.1, item_h + 30, text);
      t.attr({'font-size': 10, 'font-family': '微软雅黑'});
      var del = function() {
        per.remove();
        t.remove()
        item.bk.remove()
      ...
      return {'del': del, 'item': item, 'line': item.line}
    }
  }
