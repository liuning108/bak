define([
  "oss_core/inms/pm/items/actions/ItemAction", "text!oss_core/inms/pm/items/components/views/itemPageView.html", "oss_core/inms/pm/items/components/views/PreprocessView.js", "oss_core/inms/pm/items/components/views/addHostView.js", "oss_core/inms/pm/items/components/views/addMetric.js"
  ...
  ItemPageView.prototype.renderCom = function(d) {
    var self = this;
    this.$value_type = this.$el.find('.value_type').combobox({editable: false, dataTextField: 'paraName', dataValueField: 'paraValue', dataSource: this.dataList.ITEM_VALUE_TYPE});
    this.$value_type.combobox('value', this.dataList.ITEM_VALUE_TYPE[0].paraValue)
    this.$value_map_rule = this.$el.find('.value_map_rule').combobox({editable: false, dataTextField: 'paraName', dataValueField: 'paraValue', dataSource: this.dataList.ITEM_VALUE_MAP});
    this.$value_map_rule.combobox('value', this.dataList.ITEM_VALUE_MAP[0].paraValue)
    this.$direction = this.$el.find('.direction').combobox({editable: false, dataTextField: 'paraName', dataValueField: 'paraValue', dataSource: this.dataList.ITEM_DIRECTION});
      ...
      fish.map(data, function(d, index) {
        ...
        if (d.itemTypeList.length > 0) {
          fish.map(d.itemTypeList, function(m, key) {
            result.push({id: m.itemTypeId, pId: d.catalogId, name: m.temTypeName})
          })
        }
      })
      var options = {
    ...
    action.pmtemplateCPInfo({"templateId": ""}).then(function(data) {
      ...
      self.$pm_checkpoint = self.$el.find('.pm_checkpoint').combobox({editable: false, dataTextField: 'cpName', dataValueField: 'cpId', dataSource: data});
      self.$pm_checkpoint.combobox(
        'value', (self.upObj && self.upObj.cpId)
        ? self.upObj.cpId
        : data[0].cpId)
    })
  }
  ...
  ItemPageView.prototype.getProkeyVal = function() {
    ...
    fish.map(data, function(d) {
      var params = d.params
        ? d.params.split("|").join("\n")
        : null;
      obj.push({"type": d.edit, "paras": params})
    });
    return obj;
  }
  ...
  ItemPageView.prototype.renderItemBaseInfo = function() {
    ...
    if (self.upObj && self.upObj !== 'undefined') {
      self.$el.find('.item_name').val(
        self.upObj.itemName
        ? self.upObj.itemName
        : '');
      self.$el.find('.unit').val(
        self.upObj.unit
        ? self.upObj.unit
        : '');
      self.$el.find('.expression').val(
        self.upObj.expression
        ? self.upObj.expression
        : '');
      self.$value_type.combobox('value', self.upObj.valueType)
      self.$el.find('.comments').val(
        self.upObj.comments
        ? self.upObj.comments
        : '');
      self.$el.find('.metric_interval').val(
        self.upObj.metricInterval
        ? self.upObj.metricInterval
        : '');
  ...
  ItemPageView.prototype.getInfo = function() {
    ...
    var itemTypeId = self.$item_type_id.combotree('value')
      ? self.$item_type_id.combotree('value').id
      : ''
    var info = {
    ...
    if (self.upObj && self.upObj.itemId)
      info.itemId = self.upObj.itemId;
    if (itemName)
    ...
    if (metricInterval)
      info.metricInterval = metricInterval;
    if (itemTypeId)
    ...
    if (getPkeyVal.length > 0)
      info.preprocList = getPkeyVal;
    return info;
  }
