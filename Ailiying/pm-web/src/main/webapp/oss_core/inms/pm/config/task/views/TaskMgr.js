define([
	'text!oss_core/inms/pm/config/task/templates/TaskMgr.html',
	'i18n!oss_core/inms/pm/config/task/i18n/task',
	'oss_core/inms/pm/config/task/actions/TaskAction',
	...
	return portal.BaseView.extend({
		...
		resize: function(delta){
			this.catTree.jqGrid("setGridHeight",this.$(".js-task-left-panel").height() - this.$(".js-task-left-panel-h3").height()-40);
			this.taskGrid.jqGrid("setGridHeight",this.$(".js-task-left-panel").height() - this.$(".js-task-left-panel-h3").height()-80);
		},
		loadTree: function(){
		...
		loadGrid: function(){
			...
			this.taskGrid = $grid.jqGrid({
				...
				beforeEditRow: function(e, rowid, rowdata, option) {
					this.taskGrid.jqGrid("setSelection", rowdata);
		...
		delTask: function(value){
			value["OPER_TYPE"] = "del";
			var taskType = value["TASK_TYPE"] ;
			if(taskType=='01'){
				value["PLUGIN_TYPE"] = "02";
				value["PARAM_CODE"] = "CHECK_SERV";
			}else if(taskType=='05'){
				value["PLUGIN_TYPE"] = "05";
				value["PARAM_CODE"] = "SERVICE_SERV";
			}else if(taskType=='06'){
				value["PLUGIN_TYPE"] = "06";
				value["PARAM_CODE"] = "SERVICE_SERV";
			}
		...
		showDetailModal: function(operType,task_type,rowdata){
			...
			fish.popupView({
				url: "oss_core/inms/pm/config/task/views/TaskModal",
				viewOption: options,
