 ...
 */
define([
	'text!oss_core/inms/pm/alarmtemplate/templates/alarm/AlarmDetail.html',
	'text!oss_core/inms/pm/alarmtemplate/templates/alarm/AlarmLevelTab.html',
	'text!oss_core/inms/pm/alarmtemplate/templates/alarm/AlarmRuleStep.html',
	'oss_core/inms/pm/util/actions/UtilAction',
	'oss_core/inms/pm/meta/dim/actions/DimAction',
    ...
    return portal.BaseView.extend({
		...
		domComplete: function(){
			this.editorArr = [];
			this.fieldObj = {}; //{MODEL_PHY_CODE:[FIELD_CODE]}
			this.modelObj = {}; //{MODEL_PHY_CODE:MODEL_BUSI_NAME}
			...
			modelBusiAction.qryModelField(param, function(data) {
				...
				if (data){
					...
					fish.forEach(data.modelField,function(field){
						this.modelObj[field['MODEL_PHY_CODE']] = {'name':field['MODEL_BUSI_NAME'],'granu':field['GRANU_MODE']} ;

						if(this.fieldObj[field['MODEL_PHY_CODE']]){
			    			this.fieldObj[field['MODEL_PHY_CODE']].push(field['FIELD_CODE']);
			    		}else{
			    			this.fieldObj[field['MODEL_PHY_CODE']] = [field['FIELD_CODE']];
			    		}
			    	}.bind(this));
			...
			this.taskAction.qryDetail({"TASK_NO":this.datas.TASK_NO, 'TYPE':'param', 'PLUGIN_TYPE':'07','PARAM_CODE':'NOTIFY_OBJ'}, function(data) {
				if (data){
					...
					fish.forEach(this.modelField, function(field) {
						if(field['MODEL_PHY_CODE'] == paramObj['MODEL_CODE']
		...
		filterModel: function(isSeling){
			...
			$.each(this.fieldObj, function(modelCode, fieldArr) {
				if(this.isContained(fieldArr,checkArr)){
					modelList.push({'MODEL_PHY_CODE':modelCode,'MODEL_BUSI_NAME':this.modelObj[modelCode]['name']});
				}
			}.bind(this));
		...
		setModelCombo: function(){
			...
			fish.forEach(modelList, function(model) {
				if(model['MODEL_PHY_CODE'] == oldModelCode){
					isExist = true;
				}
			}.bind(this));
			...
			if(isExist){
				...
				this.$form.find("[name='MODEL_CODE']").combobox('value',(modelList.length > 0)?modelList[0]['MODEL_PHY_CODE']:null);
			}
		},
		...
		distinctModelField: function(isSeling){
			...
			var distinctField = [];
			fish.forEach(modelList, function(model) {
				$.merge(distinctField,this.fieldObj[model['MODEL_PHY_CODE']]);
			}.bind(this));
