...
public class GraphsDAOMysqlImpl extends GraphsDAO {
	...
	private static OpbLogger logger = OpbLogger.getLogger(GraphsDAOMysqlImpl.class, "PM");
	...
	public JSONObject getTemplateCatagorys(JSONObject dict) throws BaseAppException {
		...
		try {
			...
			logger.error("PM-Graphs-getTemplateCatagorys", e.getMessage());
			return result;
		}
	...
	public JSONObject getTemplateById(JSONObject dict) throws BaseAppException {
		...
		try {
			...
			logger.error("PM-Graphs-getTemplateById", e.getMessage());
			return result;
		}
		return result;
	}
	...
	public JSONObject getTemplatesByCatagroyId(JSONObject dict) throws BaseAppException {
		...
		try {
			...
			logger.error("PM-Graphs-getTemplatesByCatagroyId ", e.getMessage());
			return result;
		}
		return result;
	}
	...
	public JSONObject getItemsByTemplateId(JSONObject dict) throws BaseAppException {
		...
		try {
			...
			logger.error("PM-Graphs-getItemsByTemplateId ", e.getMessage());
			return result;
		}
		return result;
	}
	...
	public JSONObject getGraphsTags(JSONObject dict) throws BaseAppException {
		...
		try {
			String sql = "" + "select graphclass NAME ,count(1) CNT " + "from pm_graphy_list "
					+ "where graphclass is not null  and graphclass <>'' " + "group by graphclass "
					+ "order by CNT desc " + "LIMIT ? ";
			Long num = dict.getLong("num");
			result.put("result", this.queryForMapList(sql, new Object[] { num }));
		} catch (Exception e) {
			logger.error("PM-Graphs-getGraphsTags ", e.getMessage());
			return result;
		}
		return result;
	}
	...
	private void updateDetail(JSONObject dict, String gid) {
		// TODO Auto-generated method stub
		deleteDetail(gid);
		String sql = "" + "INSERT INTO  pm_graphy_detail " + "( graphid, " + "attrseq, " + "attr) " + "VALUES "
				+ "( ?, " + "  ?, " + " ?);";
	...
	private void updateList(JSONObject dict) {
		...
		String sql = "" + "UPDATE  pm_graphy_list " + "SET " + "name=?, " + "graphclass = ?, " + "graphtype =?, "
				+ "description = ? " + " WHERE graphid = ? ";
		this.update(sql, new Object[] { title, graphclass, gtype, desc, gid });
	}
	...
	private String insertList(JSONObject dict) {
		...
		String sql = "" + "INSERT INTO pm_graphy_list " + "(graphid, " + "template_id, " + "name, " + "graphclass, "
				...
				+ "?, " + "?, " + "?, " + "?," + "SYSDATE())";
		this.insert(sql, new Object[] { gid, templateId, title, graphclass, gtype, userId, desc });
	...
	public JSONObject getGraphsByUserID(JSONObject dict) throws BaseAppException {
		...
		try {
			if (name.length() < 0) {
				...
				String sql = ""
						+ "select graphid GID,template_id TID , name NAME, graphclass GCLASS , graphtype GTYPE  ,description DESCR ,create_time CTIME "
						+ "from pm_graphy_list where user_id=?  and template_id=? and name like ? "
			...
			logger.error("PM-Graphs-getGraphsByUserID ", e.getMessage());
			return result;
		}
	...
	public JSONObject delGraphs(JSONObject dict) throws BaseAppException {
		...
		String sql = "" + "delete from pm_graphy_list where graphid= ? and user_id= ?";
		...
		try {
			...
			logger.error("PM-Graphs-delGraphs ", e.getMessage());
			return result;
		}
		result.put("result", true);
		return result;
	...
	public JSONObject saveOrUpdateDash(JSONObject dict) throws BaseAppException {
		...
		String tid = dict.getString("tid");
		deleteDash(tid);
		String sql = "" + "INSERT INTO  pm_screen_detail " + "( tid, " + "attrseq, " + "attr) " + "VALUES " + "( ?, "
				+ "  ?, " + " ?);";
		...
		for (int i = 0; i < attrs_parts.size(); i++) {
			try {
				...
				logger.error("PM-Graphs-saveOrUpdateDash", e.getMessage());
				e.printStackTrace();
			}
		}
	...
	public JSONObject getConfigById(JSONObject dict) throws BaseAppException {
		...
		try {
			...
			logger.error("PM-Graphs-getConfigById", e.getMessage());
			return result;
		}
		return result;
	}
