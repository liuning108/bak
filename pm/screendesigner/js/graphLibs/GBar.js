    ...
    initElement: function() {
     ...
     this.attrs.datas= this.attrs.datas||this.createRandom( this.xAxisNames,10,90);
      ...
      this.top = this.paper.areaLineBar({
        ...
        'x': x,
        'y': y,
        'waring': [this.attrs.normalValue, this.attrs.urgencyValue, this.attrs.riskValue],
        'colors':v_colors,
      ...
      if(this.attrs.switch){
          ...
          this.doms['label_1'] = paper.text(x + 184, y + 195, this.attrs.normalText).attr({
            'fill': this.attrs.normalColor,
            'font-size': 18,
            'font-family': '微软雅黑',
            'font-weight': 'bold'
          });
          this.doms['label_1_rect'] = paper.rect(x + 140, y + 185, 20, 20).attr({
            'fill': this.attrs.normalColor,
            opacity: '0.4'
          });

          this.doms['label_2'] = paper.text(x + 184 + 65, y + 195, this.attrs.urgencyText).attr({
            'fill': this.attrs.urgencyColor,
            'font-size': 18,
            'font-family': '微软雅黑',
            'font-weight': 'bold'
          });
          this.doms['label_2_rect'] = paper.rect(x + 140 + 65, y + 185, 20, 20).attr({
            'fill': this.attrs.urgencyColor,
            opacity: '0.4'
          });

          this.doms['label_3'] = paper.text(x + 184 + 65 + 65, y + 195, this.attrs.riskText).attr({
            'fill': this.attrs.riskColor,
            'font-size': 18,
            'font-family': '微软雅黑',
            'font-weight': 'bold'
          });
          this.doms['label_3_rect'] = paper.rect(x + 140 + 65 + 65, y + 185, 20, 20).attr({
            'fill': this.attrs.riskColor,
            opacity: '0.4'
          });
      }

      this.doms['config'] = this.paper.text(0, y - 92, '配置').attr({
        'fill': 'red',
        'font-size': 18,
        'font-family': '微软雅黑',
        'font-weight': 'bold'
      ...
      this.doms['remove'] = this.paper.text(0, y - 72, '删除').attr({
        'fill': 'red',
        'font-size': 18,
        'font-family': '微软雅黑',
        'font-weight': 'bold'
