portal.define([
	...
	return portal.BaseView.extend({
		...
		domComplete: function(){
			...
			$(".js-alarm-detail-div").slimscroll({height:300});
		...
		loadAlarmLevelTabs:function(){
			fish.forEach(this.pmUtil.paravalue("ALARM_LEVEL"),function(para,index){
        		...
        		this.$(".js-alarm-level-content").append(this.alarmLevelTab(fish.extend({},this.i18nData,{'alarm-level':val['level']})));
        	}.bind(this));
				...
				if($(classify).attr('type')=='dim'){
					...
					if(!dimScript){
						$(classify).find(".js-alarm-o-list-ul").append(
							"<textarea name=\"ALARMOBJ_NO\" rows=\"4\"class=\"form-control\" value='"+AlarmObjNo+"' data-rule=\"{{ALARM_O}}:length[1~10240, true]\"  placeholder=\""+this.i18nData.NO_O_SEL_P_INPUT+this.i18nData.FOR_EXAMPLE+":ABC,DEF,XYZ\"></textarea>	\n"
						);
						$(classify).find(".js-alarm-o-list-add").addClass('hide');
					}else{
						utilAction.qryScriptResult({'SCRIPT':dimScript}, function(data) {
							...
							if (data && data.resultList){
								...
								if(AlarmObjNo){
									fish.forEach(AlarmObjNo.split(","), function(dimId,index) {
										
										fish.forEach(this.resultList, function(dimObj,index) {
											if(dimObj['ID'] == dimId){
												
												this.addMoItem(this.$form.find("div[classify][type='dim']"),dimId,dimObj['NAME']?dimObj['NAME']:dimId);
		...
		ok: function(){
			if (this.$form.isValid()) {
				...
				var alarmObjType = value['ALARMOBJ_TYPE'];
				if(alarmObjType){ 
					var arr = alarmObjType.split("|");
					if(arr.length > 1 ){
						value['ALARMOBJ_TYPE'] = arr[1];
					}
				}
		...
		setStepKpi: function(code,name,form){
			...
			fish.forEach(forms, function(stepForm,index) {
				...
				$(stepForm).find(".js-alarm-rule-kpi-seling-ul").slimscroll({height:140});
			});
		},
