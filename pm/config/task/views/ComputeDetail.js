portal.define([
	'text!oss_core/pm/config/task/templates/compute/ComputeDetail.html',
	'text!oss_core/pm/config/task/templates/compute/DataCheckStep.html',
	'text!oss_core/pm/config/task/templates/compute/DataMergeStep.html',
	'text!oss_core/pm/config/task/templates/compute/DataMapStep.html',
	'oss_core/pm/config/task/views/merge/MergeView',
	...
	return portal.BaseView.extend({
		...
		domComplete: function(){
			...
			$(".js-compute-detail-div").slimscroll({height:300});
		},
		...
		loadComputeDetailParam:function(){
			...
			var param = {"TASK_NO":this.datas.TASK_NO, 'TYPE':'param', 'PLUGIN_TYPE':'02','PARAM_CODE':'CHECK_SERV'} ;
								...
								if(stepObj["TASK_STEP_ID"] == stepParam["TASK_STEP_ID"]
									...
									if(code == 'CHECK_SERV'){
										...
										if(pluginObj.length > 0){
											$("input[group='CHECK_SERV']:last").val(pluginObj[0]["PLUGIN_CLASSPATH"]);
										}
		...
		checkRuleAdd:function(){
			$(".js-compute-check-rule-div").append(
				"<div class=\"col-md-12 m-t-xs js-compute-check-rule\">	"
				+"    <div class=\"input-group js-compute-check-rule-group\"> "
				+"        <input group=\"CHECK_SERV\" class=\"form-control\" data-rule=\""+this.i18nData.DATA_CHECK_RULE+":required;length[1~2048, true]\" placeholder=\""+this.i18nData.FOR_EXAMPLE+":com.ztesoft.zsmart.oss.core.pm.CheckRule\"> "
				...
				if(stepNo=="1"){//CFG_CHECK
					...
					fish.forEach($(form).find(":input[group='CHECK_SERV']"), function(check,seq) {
						if($.trim($(check).val())){
							...
							pluginObj["PLUGIN_TYPE"] = '02';
							pluginObj["PLUGIN_CLASSPATH"] = $.trim($(check).val());
