Raphael.fn.chartsProcess = function(config){
 ...
 paper.customAttributes.wrokflowarcpie = function (centerX, centerY, startAngle, endAngle, innerR, outerR) {
  ...
  inst.styles.circleStyle={'fill':colors[0],'stroke-width':0,'stroke':'none'};
  inst.styles.circle1Style={'fill':'none','stroke-width':3,'stroke':'#47494c','opacity':0.9}
  ...
  for(var i=0;i<inst.config.keys.length;i++){
        ...
        if (i<inst.config.keys.length-1){
         ...
         item.line1=paper.rect(item.x,item.y-58,inst.config.space/2,inst.config.circle_r/3).attr({'stroke-width':1,'fill':colors[0]}).toBack();
         item.line2=paper.rect(item.x+inst.config.space/2,item.y-58,inst.config.space/2,inst.config.circle_r/3).attr({'stroke-width':1,'fill':colors[0]}).toBack();
        ...
        item.nums=paper.chartsNumbser({'x':item.x,'y':item.y-55,'value':item.value,
  ...
  inst.inputData=function(datas){
   ...
   for (var i=0;i<inst.items.length;i++){
       ...
       item.circle2.animate({wrokflowarcpie: [item.x,item.y-55,0, 359*per, inst.config.circle_r+inst.config.space/9-2,inst.config.circle_r+inst.config.space/9+2]},1000);
       
        if (item.value>=inst.waring[2]){
                item.colorsSet.animate({'fill':colors[2]});
               }else if (item.value>=inst.waring[1]){
                 item.colorsSet.animate({'fill':colors[1]});
               }else{
                 item.colorsSet.animate({'fill':colors[0]});
               }
