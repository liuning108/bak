Raphael.fn.chartsPie = function(config){
     ...
     var paper=this;
     paper.customAttributes.arcpie = function (centerX, centerY, startAngle, endAngle, innerR, outerR) {
    ...
    return {path: path};
      ...
      var style={'stroke':"#ffffff",'stroke-width':"1.5",'fill':'#00000'};
   ...
   for (var i=0;i<modes.length;i++){
       modes[i].value=0;
           ...
           modes[i].path = paper.path().attr({
                ...
                "stroke-width": 2,
                'fill': modes[i].color,
                arcpie: [x,y,modes[i].sa, modes[i].ea, r3,arc_width]
            });  
       ...
       for (var i=0;i<items.length;i++){
         ...
         for (var i=0;i<items.length;i++){
            ...
            var item_y= list_y+(i*45)
                   item.label.animate({'y':item_y },1000)
                   item.path.animate({arcpie: [item.x-60,item_y+20,300,420, 10,20]},1000)
                    item.cnum.numobj.animate({'y':item_y+20 },1000)
   ...
   function drawPie(){
       ...
       for (var i =0;i<modes.length;i++){
           ...
           modes[i].path.animate({
                    arcpie: [x,y, modes[i].sa,modes[i].ea, r3,r3+width]
                }, 1500,function(){
                      glows.push(this.glow({'width':20,color:this.mycolor}));
                });
   ...
   function createListItem(mode,i) {
      ...
      var title_style= {'fill':'#fffee2','font-size':16,'font-family': '微软雅黑','font-weight':'bold'};
       node.label= paper.text(node.x+20,node.y,mode.name);
       node.label.attr(title_style);
       node.cnum=paper.chartsNumbser({'x':node.x+20,'y':node.y+20,'value':0,'format':true,
   	                               attrs: {'fill':'#fffb8e','font-size':17,'font-family': '微软雅黑','font-weight':'bold'}
                                  });
       node.path=paper.path().attr({
                "stroke": "#ffffff",
                "stroke-width": 2,
                'fill': mode.color,
                arcpie: [node.x-60,node.y+20,300,420, 10,20]
            });  
