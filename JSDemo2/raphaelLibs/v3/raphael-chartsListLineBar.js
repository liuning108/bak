 Raphael.fn.chartListLineBar = function(config){
  ...
  inst.styles.circleStyle={'stroke-width':0,'fill':'#ffffff','stroke':'none'}
  inst.config.circleR=7;
  inst.config.high=140;
  inst.styles.valueFontStyle={'fill':'#ffffff','font-size':24,'font-family': '微软雅黑','font-weight':'bold'}
  inst.styles.FontStyle2={'fill':'#ffffff','font-size':20,'font-family': '微软雅黑','font-weight':'bold'}
  ...
  function drawListBar(){
    ...
    if(!inst.listbar){
      ...
      for(var i=0;i<inst.config.keys.length;i++){
        ...
        var item_y=item.y-(inst.config.high-(25*i));
        ...
        inst.listbar[key].value=paper.chartsNumbser({'x':item_x+80+100/2,'y':item_y+30/2,'value':val,
                                                  attrs: inst.styles.FontStyle2
                                                 });
        var per_list=val/max_limit;
        inst.listbar[key].rect=paper.rect(item_x,item_y,100*per_list,30).attr({'fill':colors[i],'stroke-width':0,'stroke':'#ffffff','opacity':0.5}).toBack();
      }
    }else{
      for(var i=0;i<inst.config.keys.length;i++){
         var key= inst.config.keys[i];
         var item_x=item.x+50;
         var item_y=item.y-(inst.config.high-(25*i));
         inst.listbar[key].text.animate({'x':item_x+100/2,'y':item_y+30/2},500);
         var val=findValueByKey(item.datas,key);
         inst.listbar[key].value.setValue(val);
         inst.listbar[key].value.numobj.animate({'x':item_x+80+100/2,'y':item_y+30/2},500);
         var per_list=val/max_limit;
         inst.listbar[key].rect.animate({'x':item_x,'y':item_y,'width':100*per_list},500);
  ...
  function drawLineBar(){
    ...
    if (points.length>1){    
            ...
            if (!inst.curve){
             inst.curve=paper.path(inst.point_paths).attr({'stroke':'#7ce9ec','stroke-width':3}).toBack(  );
             var opacityAnim3 = Raphael.animation({ "50%": { opacity:0.5},
                                          "100%": { opacity:1 }
                                       }, 2000);
              inst.curve.animate(opacityAnim3.repeat(Infinity))
            }else{
              inst.curve.attr({'path':inst.point_paths});
   ...
   for (var i=0;i<inst.elements.length;i++){
    try {
         ...
         var element = inst.elements[i];
             element.nowLabel.attr({'text':item.now});
         item.x=inst.config.x+(i*inst.config.item_space);
         var per=item.value/inst.limit_max;
         var per_y=item.y-(inst.config.high*per);
         element.circleObj.attr({'cx':item.x,'cy':per_y});
         element.valueLabel.attr({'x':item.x,'y':per_y-25,'text':item.value});
  ...
  function drawElement(item){
    var element={};
        element.rect=paper.rect(item.x,item.y-inst.config.high,2,inst.config.high-5).attr({'fill':'#070707','opacity':0.5}).toBack();
