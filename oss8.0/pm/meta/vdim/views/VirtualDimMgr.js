 ...
 */
define([
        'text!oss_core/pm/meta/vdim/templates/VirtualDimMgr.html',
        'oss_core/pm/adhocdesigner/actions/AdhocAction',
        'oss_core/pm/meta/vdim/actions/VirtualDimAction',
        ...
        'css!oss_core/pm/meta/vdim/assets/virtualdim.css'
    ],
    function(mainTpl, adhocAction, vdimAction, pmUtil, adhocUtil) {
    return portal.BaseView.extend({
        ...
        render: function () {
            this.$el.html(this.vdimMainTemplate(this.i18nData));
            return this;
        },
        ...
        vdimItemDelete: function(e) {
            ...
            fish.confirm('Are you sure to delete the virtual dimension： '+vdim_name+' ?').result.then(function() {
                vdimAction.deleteVdim({VDIM_CODE:vdim_code},function(data) {
                    fish.success('Delete success !');
                    self.loadVdimList();
                    self.clearInput();
                });
            });
        ...
        addNewGroupClick: function() {
            ...
            if(this.highlightGroup!=null && !this.validateGroup(this.highlightGroup)){
                fish.toast('info', 'The condition value cannot be empty');
            }else{
        ...
        selectGroup: function(groupId) {
            if(this.$('#vdim-nogroup-detail').css('display')=="none"
                ...
                fish.toast('info', "Group condition can't be empty");
                return;
            }
            this.$('.selectableGroup li').removeClass("active");
        ...
        resize: function () {
            ...
            //
            var leftPosition = this.$el.parents(".tabs_nav").outerWidth()-435;
            var topPosition = this.$el.parents(".tabs_nav").outerHeight()-70;
        ...
        vdimOkClick: function () {
            ...
            if(this.groupList.length==0){
                fish.toast('info', 'Add one group item at least');
                return;
            }
            ...
            for(var i=0;i<this.groupList.length && !existSameName;i++){
                ...
                if(adhocUtil.trim(group.name)==''){
                    fish.toast('info', "Group name can't be empty");
                    return;
                }
            ...
            if(existSameName){
                fish.toast('info', 'A group with the same name exists');
                return;
            }
            //验证是否有未配置条件的组
            for(var i=0;i<this.groupList.length;i++){
            ...
            if(vdimType=="1"){
                ...
                adhocAction.expressionCheck({
                    ...
                    if(ret.isValid && ret.isValid=="1"){
                        ...
                        fish.toast('info', "An error expression exist");
                    }
                });
            }else {
                vdimAction.saveVdim({
        ...
        expCheck: function() {
            ...
            adhocAction.expressionCheck({
                ...
                if(ret.isValid && ret.isValid=="1"){
                    fish.toast('info', "Check success");
                }else{
                    fish.toast('info', "Check failure");
                }
            });
        },
