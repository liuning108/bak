define([
    ...
    var GRoot = Class.extend({
        ...
        getRootMaxWidth:function(arry,fontStyle){
            var max=0;
            var text="";
            ...
            var tt=this.paper.text(0, 0, text).attr(fontStyle);
        ...
        dataErrorProcess:function(delay){
            var self =this;

           if(this.error){
               if(!this.canvas.perview){
                   var gbbox = self.domsSet.getBBox(true);
                   self.gbboxError = self.paper.rect(gbbox.x, gbbox.y, gbbox.width, gbbox.height).attr({
                       'fill': 'red',
                       'fill-opacity': 0.8,
                       'stroke-width':0
                   }).toBack();
                   self.domsSet.push(self.gbboxError);
               }else{
                   setTimeout(function(){
                       var gbbox = self.domsSet.getBBox(true);
                       self.gbboxError = self.paper.rect(gbbox.x, gbbox.y, gbbox.width, gbbox.height).attr({
        ...
        getTimeVal:function() {
          ...
          console.log(this.attrs.timeType);
