 ...
 * </pre>
 *
 * 1. 直接引入，需要引入 `path/fish-desktop/third-party/pdfjs/build/pdf.js` 和 `path/fish-desktop/third-party/pdfjs/fish.pdf.js`
 * 2. `AMD` 方式引入，先定义 `paths` 配置信息，然后定义依赖引入 `path/fish-desktop/third-party/pdfjs/fish.pdf`
 *
 * ```js
 * requirejs.config({
 *     paths: {
