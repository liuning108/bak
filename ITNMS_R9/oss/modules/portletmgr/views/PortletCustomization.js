define([
	"text!modules/portletmgr/templates/PortletCustomTemplate.html",
//	"text!modules/widgetmgr/templates/WidgetCusGridBtns.html",
	...
	return portal.BaseView.extend({
		...
		afterRender: function() { //dom加载完成的事件			
			this.$cusTree = this.$(".js-grid-custom").grid({
				colModel:[{
					name: 'gridIndex',
					key: true,
					hidden: true
...
//				toolbar: [true, 'bottom'],
		...
		loadData: function(){
			widgetMgrAction.queryAllPortals(function(result){
				if(result && result.length > 0){
					for(var index=0; index<result.length; index++){
						result[index].gridIndex = "portal"+result[index].portletId + "portlet" + result[index].portalId;
						...
						if(data && data.length > 0){
							for(var i=0; i<data.length; i++){
								for(var j=0; j<result.length; j++){									
									if(data[i].portalId == result[j].portalId){										
											result[j].children.push(data[i]);
									}
								}
								data[i].gridIndex = "portal"+data[i].portletId + "portlet" +data[i].portalId;
								data[i].portalName = data[i].portletName;
							}
						}
		...
		bindPortlets: function() {
			...
			for(var i=0;i<that.treeResult.length;i++){
				if(that.treeResult[i].portalName == rd.portalName){
			...
			fish.popupView({
            	url:'modules/portletmgr/views/grant/BindPortlets',
            	viewOption: initData,
            	close: function(msg) {
					//不全量刷新，只更新选中跟节点下的数据。
					for(var i=0;i<that.treeResult.length;i++){
		...
		resize: function(delta) {
        	portal.utils.gridIncHeight(this.$(".js-grid-custom"), $(".portlet-left").height() - $(".portlet-right").height());
