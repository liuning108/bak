define([
	...
	return portal.BaseView.extend({
		...
		events: {
			...
			"click .js-approve": "approveBulletin",
		...
		afterRender: function() {
			...
			this.$grid.grid("navButtonAdd",[{
                caption: i18nBulletin.BULLETIN_APPROVE,
                cssprop: "js-approve"           
			}]);	
			this.$(".js-approve").prop("disabled",false);
			this.pageData(true);
		},
		...
		approveBulletin: function(){
			var that = this;
			var selrow = that.$grid.grid("getSelection");
			fish.popupView({
				url: "bulletin/modules/bulletinmgr/views/BulletinApprovePopWin",
				viewOption: {
					data: selrow
				},
				close: function(msg) {					
					...
					bulletinAction.modBulletin(msg,function(data){
						if (data){
							// var selrow = that.$grid.grid("getRowData",row.rowid);
							var nextrow = that.$grid.grid("getNextSelection", selrow); //获取下一条数据
							if (nextrow == null) {
								nextrow = that.$grid.grid("getPrevSelection", selrow); //获取上一条同级数据
							}
							...
							if (nextrow != null) {
                                ...
                                that.$(".js-approve").prop("disabled",true);
                            }
							// that.$grid.grid("setRowData", msg);
							fish.success(i18nBulletin.BULLETIN_APPROVAL_SUCCESS);
						}
					});				
				}
			});
		...
		// 	var rowdata = that.$grid.grid('getSelection');			
		...
		pageData: function(reset) {
			...
			// qryCondition.auditStaffId = portal.appGlobal.get("staffId");
			qryCondition.state = 'C';
					...
					function(data) {
						if(data){
							...
							if (bulletinList.length > 0) {
                                ...
                                that.$(".js-approve").prop("disabled",false);
                            } else {
