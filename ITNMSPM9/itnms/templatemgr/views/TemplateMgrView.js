define(
	[
		...
		return fish.View.extend({
			...
			initTemplateGrid: function () {
				...
				var opt = {
					colModel: [{
						...
						formatter: function (cellval, opts, rwdat, _act) {
							...
							return'<div class="groupMoreOp">' +
								...
								'<li role="presentation"><a style="color:#646464" class="tm-templatemore-host" id="hostbtn-' + templateid + '" role="menuitem" tabindex="-1" href="#">监测点</a></li>' +
					...
					gridComplete: function() {
						...
						//监控点
						self.$el.find(".tm-template-grid").find('.groupMoreOp').find('.tm-templatemore-host').bind("click", function(e){
							var templateid = e.currentTarget.id.substring(8);//hostbtn-xxx
							self.showHostView(templateid);
						});
			...
			qryTemplate: function () {
				...
				templateAction.qryTemplate(function(retJsonObj) {
					if(retJsonObj && retJsonObj.result){
						...
						if(self.option.hostid){
							self.filterTplByHost(self.option.hostid);
						}
					}
				});
			},

			filterTplByHost: function (hostid) {
				var self = this;
				var filterTemplateList = [];
				fish.forEach(self.templateList, function (template) {
					var flag = false;
					fish.forEach(template.hosts, function(host){
			...
			showHostView: function (templateid) {
				var self = this;
				self.$('#tm-homepage').hide();
				self.$('#tm-template-host').show();
				if(self.hostListView) return;
				require([
					'oss_core/itnms/host/components/views/HostListView'], function (HostListView)
				{
					self.hostListView = new HostListView({
						el: self.$el.find('#tm-template-host'),
						'tableH': self.leftTreeHeight-140,
						'groups': [],
