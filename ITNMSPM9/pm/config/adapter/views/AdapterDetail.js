portal.define([
	'text!oss_core/pm/config/adapter/templates/AdapterDetail.html',
	...
	return portal.BaseView.extend({
		...
		afterRender: function(){
			...
			this.moObj = {};
			this.moList = [];
			$(".js-mo-list-ul").slimscroll({height:272,width:230});
			$(".js-mo-list-seling-item").slimscroll({height:245,width:200});
			...
			if(this.EMS_TYPE_REL_ID && this.EMS_VER_CODE){
				var param = {"EMS_TYPE_REL_ID":this.EMS_TYPE_REL_ID,"EMS_VER_CODE":this.EMS_VER_CODE,"MODEL_TYPE":'0'} ; //Detail model
		...
		ok: function(){
			if (this.$form.isValid()) {
				...
				value["pluginParam"] = this.paramGrid.jqGrid("getRowData"); 
				value["adapterMO"] = [];
				value["dbDialect"] = [];
				var isValid = true;
				var tipInfo = "";
		...
		setMOObj: function(key,value){
	        ...
	        if(moCode){
	        	if(!this.moObj[moCode]) this.moObj[moCode] = {'MODEL_CODE':'','GRANU':''};
		...
		getAdapterInfo: function(){
			...
			var param = {"ADAPTER_NO":this.datas.ADAPTER_NO,"PLUGIN_NO":this.datas.PLUGIN_NO,"PLUGIN_TYPE":this.pluginType};
			this.adapterAction.qryMapping(param, function(data) {
				if (data){
					...
					fish.forEach(data.adapterMO, function(mo,index) {
		        		...
		        		if(!this.moObj[moCode]) this.moObj[moCode] = {'MODEL_CODE':mo["MODEL_CODE"] ,'GRANU':mo["GRANU"]};
		...
		showBlockUI: function(show){ 
			if(show){
				...
				this.$(".js-adapter-map-detail").blockUI({baseZ:1,blockMsgClass:'fade'}).data('blockui-content', true);
