define([
  ...
  "text!oss_core/pm/screendesigner/js/graphLibs/mapBase/GMapSplashesConfig.html",
  ...
  return portal.CommonView.extend({
    ...
    $parent.find('.isGlow').prop('checked', this.gText.attrs.showGradient === true).off('click').on('click', function() {
      ...
      $parent.find('.showGradientValue')
             ...
             $parent.find('.pointSize')
                    .val(this.gText.attrs.pointSize)
                    .off('change')
                    .on('change',function(){
                       var value = $(this).val();
                       if(value.length<=0) value=20;
                       value = Number(value);
                       if(isNaN(value))value=20;
                       $(this).val(value);
                       self.gText.attrs.pointSize=value;
                       self.gText.redraw();
                    })
      $parent.find('.mapGtype').val(this.gText.attrs.gtype)
             .off('change')
             .on('change',function(){
               self.gText.attrs.gtype = $(this).val();
                self.gText.redraw();
             })
      ...
      var num_colorpicker1 = $parent.find(".pc1").colorpicker();
      num_colorpicker1.colorpicker("set", this.gText.attrs.pointFcolor);
      num_colorpicker1.on("move.colorpicker", function(e, color) {
        self.gText.attrs.pointFcolor = "" + color;
        self.gText.redraw();
      })


      var num_colorpicker2 = $parent.find(".pc2").colorpicker();
      num_colorpicker2.colorpicker("set", this.gText.attrs.pointScolor);
      num_colorpicker2.on("move.colorpicker", function(e, color) {
        self.gText.attrs.pointScolor = "" + color;
        self.gText.redraw();
      })
