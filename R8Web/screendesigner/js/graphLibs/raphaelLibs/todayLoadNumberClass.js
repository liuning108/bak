...
todayLoadNumberClass.prototype.show=function(){
    ...
    function getMaxWidth(keys){
        ...
        var maxText =paper.text(0,0, max).attr({'font-size':25,'font-family': '微软雅黑','font-weight':'bold'})
        var box =maxText.getBBox(true);
        maxText.remove();
        var width =box.width
        return width;
    }
      var names= fish.pluck(this.elements,'name');
      console.log(names);
      var maxWidth=getMaxWidth(names)/3;
      for ( var i = 0 ;  i<this.elements.length ; i++){
	      this.darwElem(i,maxWidth);
	  }


}

todayLoadNumberClass.prototype.darwElem = function(index,textW){
        ...
        var rect_3g =paper.rect(this.config.x+40+textW,this.config.y+interval,(this.config.element_width-40)/2*rate.value3,this.config.element_high);
        this.set.push(rect_3g);
        rect_3g.attr({'fill':this.config.c1Color,'stroke-width':0,'fill-opacity':1});
        var rect_3g_value= paper.text(this.config.x+textW+(this.config.element_width)/2-40/2,this.config.y+interval+11,element.value_3g);
        ...
        var rect_4g =paper.rect(this.config.x+textW+(this.config.element_width)/2+21,this.config.y+interval,(this.config.element_width-39)/2*rate.value4,this.config.element_high);
        this.set.push(rect_4g);
        rect_4g.attr({'fill':this.config.c2Color,'stroke-width':0,'fill-opacity':1});
        var rect_4g_value= paper.text(this.config.x+textW+(this.config.element_width)-35,this.config.y+interval+11,element.value_4g);
...
sumkpi.prototype.show =function(){
  ...
  for (var i = 0 ; i<this.config.nums;i++){
    ...
    var trident_y = this.config.y+(this.config.nums-1)*this.config.space_high;//((target_number-1)*this.config.space_high)

     var lable_title= this.paper.text(trident_x+this.config.item_width/2,trident_y+30,this.config.currentvalue);
     self.set.push(lable_title);
       lable_title.attr({'fill':'#ffffff','font-size':18,'font-family': '微软雅黑','font-weight':'bold'})
