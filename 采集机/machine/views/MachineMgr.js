    ...
    return portal.BaseView.extend({
      ...
      afterRender: function() {
        ...
        var opt = {
          ...
          subGridRowExpanded: function(e, subGridId, parentRowId) {
            ...
            if (!eidtFlag) {
              ...
              var $subGrid = $("#" + subgrid_table_id).grid({
                colModel: [{
                    name: "TASK_NO",
                    label: self.i18nData.TASK_NO,
                    width: 80,
                    key: true
                  },
                  {
                    name: "TASK_NAME",
                    label: self.i18nData.TASK_NAME,
                    width: 130
                  },
                ],
                rowNum: 20,
          ...
          beforeEditRow: function(e, rowid, data, option) {
            if ($grid.data("editGird")) {
              fish.toast('info', self.i18nData.NEED_CONTINUE_OPER);
              return false;
            }
          ...
          beforeSaveRow: function(e, rowid, data, option) {
            ...
            if (!$("#" + rowid).data("saveRow")) {
              ...
              machineMgrAction.saveOrUpdate({
                'meachineData': data,
                'tasks': tasklist
              }, function(result) {
                if (result.result_userip) {
                  fish.toast('info', self.i18nData.MACHINE_USER_IP_EXIST);
                  return;
                }
                if (result.result_dis) {
                  fish.toast('info', self.i18nData.DEFAULT_REMAIN_EXIST);
                  return;
                }
          ...
          beforeDeleteRow: function(e, rowid, data, option) {
            ...
            fish.confirm(self.i18nData.CONFIRM_DELETE).result.then(function() {
        ...
        $grid.grid("navButtonAdd", [{
          title: self.i18nData.COMMON_NEW,
          caption: self.i18nData.COMMON_NEW,
          id: "grid01_addIcon",
          buttonicon: "",
          navpos: "bottombar",
          onClick: function() {
