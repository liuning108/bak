...
define([
    ...
    return portal.BaseView.extend({
        ...
        afterRender: function () {
            ...
            action.cacheModelData({}, this.wrap(function (data) {
                ...
                action.cacheMapType({}, function (data) {
                    if(data && data.mapTypeList && data.mapTypeList.length>0){
                        self.cachedMapTypeList = [];
                        fish.forEach(data.mapTypeList, function(mapType){
                    ...
                    fish.forEach(this.chartList, this.wrap(function (view) {// 对所有图表进行更新操作
                        ...
                        if(tagType==0) {// 维度删除
                            ...
                            if(view.chart_type=="map" || view.chart_type=="scattermap"){
                                this.refreshMapColorPanel(view);
                            }else {
        ...
        showMapFieldCfg: function () {
            if(this.highlightChart) {
                portal.require([
                    'oss_core/pm/adhocdesigner/views/MapFieldCfg'
                ], this.wrap(function (Dialog) {
                    var sData = {
                        axisCfgXaxis: this.highlightChart.axisCfgXaxis,
                        axisCfgYaxisList: this.highlightChart.axisCfgYaxisList,
        ...
        showMapColorCfg: function() {
            if(this.highlightChart) {
                portal.require([
                    'oss_core/pm/adhocdesigner/views/MapColorCfg'
                ], this.wrap(function (Dialog) {
                    var sData = {
                        mapColorCfg: this.highlightChart.mapColorCfg
                    };
                    var dialog = new Dialog(sData);
                    var content = dialog.render().$el;
                    var option = {
                    ...
                    this.mapColorCfgView = fish.popup(option);
                    dialog.contentReady();
                    this.listenTo(dialog, 'okEvent', this.wrap(function (data) {
                        this.highlightChart.mapColorCfg = data.mapColorCfg;
                        this.refreshChart(this.highlightChart);
                        this.mapColorCfgView.close();
                    }));
                    this.listenTo(dialog, 'cancelEvent', this.wrap(function () {
                        this.mapColorCfgView.close();
                    }));
        ...
        showMapTypeCfg: function () {
            if(this.highlightChart) {
                portal.require([
                    'oss_core/pm/adhocdesigner/views/MapTypeCfg'
                ], this.wrap(function (Dialog) {
                    var sData = {
                        mapTypeList: this.cachedMapTypeList,
                        AREAMAP_NO: this.highlightChart.AREAMAP_NO
                    };
                    var dialog = new Dialog(sData);
                    var content = dialog.render().$el;
                    var option = {
                    ...
                    this.mapTypeCfgView = fish.popup(option);
                    dialog.contentReady();
                    this.listenTo(dialog, 'okEvent', this.wrap(function (data) {
                        this.highlightChart.AREAMAP_NO = data.AREAMAP_NO;
                        this.highlightChart.AREAMAP_URL = data.AREAMAP_URL;
                        this.refreshChart(this.highlightChart);
                        this.mapTypeCfgView.close();
        ...
        mappingLegendDirectionClass: function (legendDirection) {
            ...
            switch (legendDirection) {
                case "vertical-left-center" : className = "a1";break;
                case "vertical-right-center" : className = "a2";break;
                case "horizontal-center-top" : className = "a3";break;
                case "horizontal-center-bottom" : className = "a4";break;
            }
            return className;
        },
