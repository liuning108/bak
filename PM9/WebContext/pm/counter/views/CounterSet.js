portal.define([
	'text!oss_core/pm/counter/templates/CounterSet.html',
	'oss_core/pm/meta/measure/actions/MeasureAction',
	"css!oss_core/pm/util/css/space-grid.css",
	...
	return portal.BaseView.extend({
		...
		afterRender: function(){
			...
			this.$(".js-counter-seling-ul").slimscroll({height:250});
			this.$(".js-counter-seled-ul").slimscroll({height:250});
			
			this.loadEmsType();
			this.$form.find("input[name='EMS']").combobox();
			this.$form.find("input[name='EMS_VER']").combobox();
		...
		loadEmsType: function(){
			...
			this.$form.find("input[name='EMS_TYPE']").combobox({
				dataTextField: 'EMS_TYPE',
		        dataValueField: 'EMS_TYPE_CODE',
		        dataSource:  emsTypeInfo
			});
			this.$form.find("input[name='EMS_TYPE']").on('combobox:change', function () {
				this.$form.find("input[name='EMS']").combobox('value','');
				this.loadEms();
			}.bind(this));
		...
		loadEms:function(ems){
			...
			var emsInfo = [];
			fish.forEach(this.emsData['emsList'], function(ems) {
				if(ems['EMS_TYPE_CODE'] == emsType && ems['EMS_CODE']!='ANY_VENDOR'){ //不显示 Any Vendor
			...
			this.$form.find("input[name='EMS']").on('combobox:change', function () {
				this.$form.find("input[name='EMS_VER']").combobox('value','');
				this.loadEmsVer();
			}.bind(this));
			if(ems){
				this.$form.find("input[name='EMS']").combobox('value',ems);
			}else{
				this.$form.find("input[name='EMS']").combobox('value',(emsInfo.length > 0)?emsInfo[0]['EMS_CODE']:'');
			}
		},
		loadEmsVer: function(ver){
			var ems = this.$form.find("input[name='EMS']").combobox('value');
			var verInfo = [];
			fish.forEach(this.emsData['verList'], function(ver) {
				if(ver['EMS_CODE'] == ems){
					verInfo.push({'EMS_VER_NAME':ver['EMS_VER_NAME'],'EMS_VER_CODE':ver['EMS_VER_CODE']}) ;
				}
			});
			this.$form.find("input[name='EMS_VER']").combobox('destroy');
			this.$form.find("input[name='EMS_VER']").combobox({
				dataTextField: 'EMS_VER_NAME',
		        dataValueField: 'EMS_VER_CODE',
		        dataSource:  verInfo
			});
			
			this.$form.find("input[name='EMS_VER']").on('combobox:change', function () {
				this.$form.find("input[name='MO_CODE']").combobox('value','');
				this.loadMo();
			}.bind(this));
			if(ver){
				this.$form.find("input[name='EMS_VER']").combobox('value',ver);
			}else{
				this.$form.find("input[name='EMS_VER']").combobox('value',(verInfo.length > 0)?verInfo[0]['EMS_VER_CODE']:'');
			}
		...
		init:function(){
			...
			fish.forEach(this.counterData['detail'], function(detail) {
				this.counterSeledAppend(detail['code'],detail['name'],detail['title'],detail['MO_CODE'],detail['MO_NAME'],detail['MODEL_CODE']);
			}.bind(this));
		},
			...
			this.pmUtil.utilAction.qryScriptResult({SCRIPT:sql},function(data) {
				if(data && data.resultList){ 
					...
					fish.forEach(data.resultList,function(mo_field){
						...
						this.counterSelingAppend(code, name, title, mo_field['MO_CODE'], mo_field['MO_NAME'],mo_field['MODEL_CODE'])
					}.bind(this));
				}
			}.bind(this),true);		
		},
		ok: function(){
			...
			fish.forEach(this.$(".js-counter-seled-ul .js-counter-del"), function(item,index) {
				...
				var modelCode = $(item).attr("modelCode") ;
				var moCode = $(item).attr("moCode") ;
				var moName = $(item).attr("moName") ;
				retData['counter'] += '['+code+']';
				retData['detail'].push({'code':code,'name':name,'title':title,
					'FIELD_CODE':code, 'FIELD_NAME':name,'MODEL_CODE':modelCode,
					'MO_CODE':moCode, 'MO_NAME':moName});
			}.bind(this));
		...
		counterAdd:function(event){
			...
			this.counterSeledAppend($target.attr('code'),
				$target.attr('name'),
				$target.attr('tt'),
				$target.attr('moCode'),
				$target.attr('moName'),
				$target.attr('modelCode') );
		},
		counterDel:function(event){
			var $target = $(event.target) ;
			$target.parent().remove();
			this.counterSelingAppend($target.attr('code'),
				$target.attr('name'),
				$target.attr('tt'),
				$target.attr('moCode'),
				$target.attr('moName'),
		...
		counterAddAll:function(){
			fish.forEach(this.$(".js-counter-seling-ul .js-counter-add"), function(item,index) {
				this.counterSeledAppend($(item).attr("code"),
					$(item).attr("name"),
					$(item).attr("tt"),
					$(item).attr("moCode"),
					$(item).attr("moName"),
		...
		counterDelAll:function(){
			fish.forEach(this.$(".js-counter-seled-ul .js-counter-del"), function(item,index) {
				this.counterSelingAppend($(item).attr("code"),
					$(item).attr("name"),
					$(item).attr("tt"),
					$(item).attr("moCode"),
					$(item).attr("moName"),
		...
		counterSelingAppend:function(code,name,title, moCode, moName, modelCode){
			this.$(".js-counter-seling-ul").append("<li class=\"ui-sortable-handle\" title='"+title+"'>"
				...
				+"<a href=\"#\" class=\"js-counter-add\" code='"+code+"' name='"+name+"' tt='"+title+"' moCode='"+moCode+"' moName='"+moName+"' modelCode='"+modelCode+"'>"+this.i18nData.COMMON_ADD+"</a></li>");
		},
		counterSeledAppend:function(code,name,title, moCode, moName, modelCode){
			this.$(".js-counter-seled-ul").append("<li class=\"ui-sortable-handle\" title='"+title+"'>"
				+"<span style='width:195px;overflow:hidden; text-overflow:ellipsis; white-space:nowrap; word-break:keep-all;'>"+name+"</span>"
				+"<a href=\"#\" class=\"js-counter-del\" code='"+code+"' name='"+name+"' tt='"+title+"' moCode='"+moCode+"' moName='"+moName+"' modelCode='"+modelCode+"'>"+this.i18nData.COMMON_DELETE+"</a></li>");
		},
