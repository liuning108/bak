define([
        "text!oss_core/pm/config/task/templates/compute/merge/TableView.html",
        "text!oss_core/pm/config/task/templates/compute/merge/CardView.html",
        "text!oss_core/pm/config/task/templates/compute/merge/NullView.html",
        "text!oss_core/pm/config/task/templates/compute/merge/checkbox.html",
        "text!oss_core/pm/config/task/templates/compute/merge/checkboxOutput.html",
        "css!oss_core/pm/util/css/ad-util-component.css",
        "oss_core/pm/config/task/views/merge/js/jquery.widget.swappable"
    ],
    function(tpl, cardViewTpl, nullView, checkboxView, checkboxViewOutput) {
        return portal.CommonView.extend({
            ...
            putAll:function(datas,isGrouping){
               var self =this;
               fish.each(datas,function(data){self.putModel(data,isGrouping)});
            },
            ...
            addRow: function(model,isGrouping) {
                ...
                var body = self.tableTemplate.td_header({'no': model.no, 'name': model.name });
            ...
            addLastRow: function(td_nums) {
                ...
                var domStr = self.createRowHTML({
                    ...
                    handel: function(key, value) {
                        return self.tableTemplate.td_operation_output({ 'name': this.i18nData.OUTPUT, 'flag_class': 'output_checkbox', 'flag_output_class': 'output_text' });
                    }.bind(this),
                });
                self.tableBody.append(domStr);
                /*
            ...
            addFristRow: function(td_nums) {
                ...
                var domStr = self.createRowHTML({
                    ...
                    datas: fish.range(td_nums),
                    handel: function(key, value) {
                        return self.tableTemplate.td_operation({ 'name': this.i18nData.MERGE, 'flag_class': 'notnull_checkbox' });
                    }.bind(this),
                });
            ...
            createRowHTML: function(option) {
                var self = this;
                var body = self.tableTemplate.td_header({ 'no': option.no, 'name': option.name });
