  ...
  "oss_core/pm/screendesigner/js/graphLibs/mapBase/MapBase",
   ...
   var MapSplashes  = MapBase.extend({
       ...
       for (var i = 0; i < self.attrs.mapDatas.length; i++){
         ...
         if (showTitle) {
           ...
           if(self.doms["map_" + data.id + "_text"]){
             self.doms["map_" + data.id + "_text"].attr({'text':title}).toFront();
           }else{
         ...
         if (showValue) {
           ...
           if(self.doms["map_" + data.id + "_value"]){
             self.doms["map_" + data.id + "_value"].attr({"text":data.value}).toFront();
           }else{
     ...
     createlightRing :function(data,sclr,fclr,max,r,i){
       ...
       if(self.doms['map_circle_pointe'+data.id]){
         self.doms['map_circle_pointe'+data.id].remove();
         self.doms['map_circle'+data.id].remove();
         self.doms['map_circle_glow'+data.id].remove();
       }
     ...
     createPoint:function(data,sclr,fclr,max,r){
       ...
       var y = bbox.y + bbox.height / 2;
       var title = self.doms["map_" + data.id].title || data.id;
       var per = data.value/max; //data.value
       var dyR= r * per;
       var color = self.pickHex(sclr, fclr, per)
       if(self.doms['map_circle'+data.id]){
           self.doms['map_circle'+data.id].remove();
       }
       self.doms['map_circle'+data.id]=self.paper.circle(x,y,dyR).attr({'fill':color,'stroke-width':0,'opacity':0.9});
      if(self.doms['map_circle_glow'+data.id])self.doms['map_circle_glow'+data.id].remove();
       self.doms['map_circle_glow'+data.id]= self.doms['map_circle'+data.id].glow({
         'color':color,
       })
     },
