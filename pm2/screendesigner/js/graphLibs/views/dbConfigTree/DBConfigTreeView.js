define([
    ...
    **/
        ...
        changeDB: function() {
            ...
            var $edit_btn = $parent.find('.db_edit_btn');
            var $ul = $parent.find('.db_edit_plane').find('.dbserverlistUL');
            var $actionLi = $ul.find(".actionService");
            if($actionLi.length<=0){
                 fish.warn("Please select a service")
                return;
            }
            ...
            g.attrs.dbServer={
                ...
                'yMinNums' : this.config.g.attrs.dbServer.yMinNums
            }
            var $parent = $(".db_panel_side");
            var $edit_btn = $parent.find('.db_edit_btn');
            //
            dbHelper.getServiceDataInfo(g).done(function(){
               self.config.db = dbHelper.getJson(g);
               self.afterRender();
               $edit_btn.click();
               $parent.find('.db_edit_plane').hide();
            })
        ...
        afterRender: function() {
            ...
            $parent.find('.g_field_context').slimscroll({height: outH, axis: 'y',width:"300px"});
        }
