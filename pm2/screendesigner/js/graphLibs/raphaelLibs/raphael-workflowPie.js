(function(root, factory) {
    ...
    Raphael.fn.workflowPie = function(config) {
        ...
        paper.customAttributes.arcpie = function(centerX, centerY, startAngle, endAngle, innerR, outerR, value) {
            ...
            if (!this.circle) {
                this.circle = paper.circle(c_outerX2, c_outerY2, 7).attr({
                    'fill': 'none',
                    'stroke': color,
                    'stroke-width': 4,
                    'opacity': 0.7
                });
                obj_instance.set.push(this.circle);
                this.linePath = paper.path([
                    ["M", c_outerX1, c_outerY1],
                    ["L", l_outerX2, l_outerY2]
                ]).attr(pathParams);
                obj_instance.set.push(this.linePath);
                this.nameLabel = paper.text(t_outerX2, t_outerY2, this.name).attr({
                    'fill': color,
                    'font-size': 22,
                    'font-family': '微软雅黑',
                    'font-weight': 'bold'
                });
                obj_instance.set.push(this.nameLabel);
                var fontw = this.name.length * 22;
                this.nameValue = paper.text(v_outerX2 + fontw, v_outerY2, this.value).attr({
                    'fill': '#ffffff',
                    'font-size': 22,
                    'font-family': '微软雅黑',
                    'font-weight': 'bold'
                });
                obj_instance.set.push(this.nameValue);
            } else {
                this.circle.attr({
                    'cx': c_outerX2,
                    'cy': c_outerY2,
        ...
        var c1 =paper.circle(x, y, arc_width + 85).attr({
            'fill': 'none',
            'stroke': '#9cb722',
            'stroke-width': 1.5,
            'opacity': 0.35
        });
        obj_instance.set.push(c1);
        var c2 =paper.circle(x, y, arc_width + 50).attr({
            'fill': 'none',
            'stroke': '#9cb722',
            'stroke-width': 1.5,
            'opacity': 0.50
        });
        obj_instance.set.push(c2);
        var c3 =paper.circle(x, y, arc_width + 25).attr({
            'fill': 'none',
            'stroke': '#9cb722',
            'stroke-width': 1.5,
            'opacity': 0.85
        });
        obj_instance.set.push(c3);
