define([
    "oss_core/pm/screendesigner/js/graphLibs/GRoot", "oss_core/pm/screendesigner/js/graphLibs/views/GNodesView", "oss_core/pm/screendesigner/js/graphLibs/raphaelLibs/raphael-chartsProcess"
], function(GRoot, View) {
    ...
    var GNodes = GRoot.extend({
        ...
        initElement: function() {
            ...
            this.attrs.names = this.attrs.names || [
                'CRM下单',
                '服务单',
                '资源变更单',
                '流程启动',
            ...
            this.doms['config'] = this.paper.text(-10, 30, '配置').attr({'fill': 'red', 'font-size': 18, 'font-family': '微软雅黑', 'font-weight': 'bold'});
            this.doms['remove'] = this.paper.text(30, 30, 'X').attr({'fill': 'red', 'font-size': 20, 'font-family': '微软雅黑', 'font-weight': 'bold'});;
        },
        ...
        createIconTextItem: function(i, box, space, y, texts) {
            ...
            // draw text
            this.doms['label' + i] = paper.text(item_x, y + 30, texts[i]).attr({'fill': this.getColors(i), 'font-size': 18, 'font-family': '微软雅黑', 'font-weight': 'bold'})
            box = this.doms['label' + i].getBBox(true);
            //draw icon
            this.doms['icon' + i] = paper.rect(0, y + 20, 20, 20).attr({'fill': this.getColors(i), 'stroke-width': 0})

            var iconBox = this.doms['icon' + i].getBBox();
            this.doms['icon' + i].attr({
                'x': box.x - (iconBox.width + space)
            });
        ...
        setSwitchWarring: function(checked) {
            this.attrs.switchWarring = checked
                ? 'on'
                : 'off';
            this.redraw();
        },
